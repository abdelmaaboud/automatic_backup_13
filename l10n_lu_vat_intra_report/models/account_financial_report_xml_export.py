...
class AccountFinancialReportXMLExport(models.TransientModel):
    _inherit = "account.financial.html.report.export"

    @api.model
    def is_xml_export_available(self, report_obj):
        if report_obj._name == 'l10n.lu.report.partner.vat.listing' or report_obj._name == 'l10n.lu.report.partner.vat.intra' or (report_obj._name == 'account.financial.html.report' and report_obj.id == self.env['ir.model.data'].xmlid_to_res_id('l10n_lu_reports.account_financial_report_l10n_lu_tva0')):
            return True
        else:
            return super(AccountFinancialReportXMLExport, self).is_xml_export_available(report_obj)

    def do_xml_export(self, context):
        if context.get_report_obj()._name == 'account.financial.html.report' and context.get_report_obj().id == self.env['ir.model.data'].xmlid_to_res_id('l10n_lu_reports.account_financial_report_l10n_lu_tva0'):
            return self._l10n_lu_vat_export(context)
        if context.get_report_obj()._name == 'l10n.lu.report.partner.vat.listing':
            return self._l10n_lu_partner_vat_export(context)
        if context.get_report_obj()._name == 'l10n.lu.report.partner.vat.intra':
            return self._l10n_lu_partner_vat_intra_export(context)
        else:
            return super(AccountFinancialReportXMLExport, self).do_xml_export(context)

    @api.model
    def check(self, report_name, report_id=None):
        if report_name == 'account.financial.html.report' and report_id == self.env['ir.model.data'].xmlid_to_res_id('l10n_lu_reports.account_financial_report_l10n_lu_tva0'):
            ...
            return True
        if report_name == 'l10n.lu.report.partner.vat.listing':
            company = self.env.user.company_id
            vat_no = company.partner_id.vat
            if not vat_no:
                return _('No VAT number associated with your company.')
            ...
            if not address.phone:
                return _('No phone associated with the company.')
            return True
        if report_name == 'l10n.lu.report.partner.vat.intra':
            company = self.env.user.company_id
            vat_no = company.partner_id.vat
            if not vat_no:
                return _('No VAT number associated with your company.')
    ...
    def _l10n_lu_partner_vat_intra_export(self, context):
...
<ns2:IntraConsignment xmlns="http://www.minfin.fgov.be/InputCommon" xmlns:ns2="http://www.minfin.fgov.be/IntraConsignment" IntraListingsNbr="1">
    ...
    def _l10n_lu_partner_vat_export(self, context):
        seq_declarantnum = self.env['ir.sequence'].get('declarantnum')
        company = self.env.user.company_id
        company_vat = company.partner_id.vat
