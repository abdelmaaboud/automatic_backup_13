...
#         return fields.Date.from_string(self.end_date or fields.Date.to_string(datetime.today()))
    
    #get context from method print_timesheets_report in sale.subscription (contract_report)
    @api.multi
    def action_print(self):
        self.update_subscription()
        return self.env.ref('contract_report.contract_abakus').report_action(self.subscription_id)
    ...
    def update_subscription(self):
        self.subscription_id.write({
            'report_start_date': self.start_date or self.subscription_id.date_start,
            'report_end_date': self.end_date or fields.Date.to_string(datetime.today()),
            'report_remove_prices': self.remove_prices,
            'report_only_worklogs': self.only_worklogs,
            'report_group_by_issue_and_task': self.group_by_issue_and_task,
        })